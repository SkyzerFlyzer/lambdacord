...
// One curl round trip. Throws ModuleError(upstream) on transport failure.
...
// Executes `method url` with retries per `policy`:
//   - 2xx returns RestResponse{status, body}.
//   - 429 sleeps min(parse_retry_after(...), remaining wait budget) and
//     retries; a sleep that would exceed the remaining budget is NOT taken —
//     throws instead.
//   - 5xx retries with backoff_ms(attempt) under the same budget rule.
//   - Non-429 4xx throws immediately, status in safe_context.
...
// never surface them to Discord users (CLAUDE.md error-copy rule).
inline RestResponse discord_request(const std::string& method, const std::string& url,
    ...
    long remaining_wait_ms = policy.max_total_wait_ms > 0 ? policy.max_total_wait_ms : 0;

    long last_status = 0;
    for (int attempt = 0; attempt < max_attempts; ++attempt) {
        const rest_detail::AttemptResult result =
            rest_detail::perform_once(method, url, request_body, has_body);
        ...
        if (result.status >= 400 && result.status < 500 && result.status != 429) {
            throw ModuleError::make(
                ...
                json{{"body", result.body}, {"url", url}});
        }

        // 429 or 5xx: retryable if attempts and wait budget remain.
        ...
        if (!last_attempt) {
            ...
            // AD-8: never take a sleep that exceeds the remaining budget.
        }
        break;
    }
